...
for (let i = 0; i < 10; i++) {
	...
	$.ajax({
		url: 'reserveSeatList.do',
		type: 'post',
		success: function(rSL) {
		var test = JSON.parse(rSL);
		console.log(test);
		for(var item in test) {
		console.log(test[item].reserve_selectedseat);
		document.querySelector("#"+test[item].reserve_selectedseat).classList.add('reserved');
		}
		}
		});
...
function inputClickEvent(input) {
    ...
    input.addEventListener('click', function(e) {
        ...
        if (input.classList.contains('reserved')) {
        	toastr.error(
        	'<div style="color:white">예약된 자리입니다</div>', {
        		timeOut: 4000
        	}
        	);
        
        } else if (input.classList.contains('clicked')) {
